...
impl Air for ThinDilMulShowAir {
    ...
    fn new(trace_info: TraceInfo, pub_inputs: Self::PublicInputs, options: ProofOptions) -> Self {
        ...
        main_degrees.append(&mut vec![TransitionConstraintDegree::with_cycles(2, vec![PADDED_TRACE_LENGTH]); SWAP_C_TRIT - SWAP_FE_EQUAL_IND]); // ranges 131-328
    ...
    fn evaluate_transition<E: FieldElement + From<Self::BaseField>>(
        ...
        // Copy m
        for i in 0..HASH_DIGEST_WIDTH {
            result[M_IND+i] += next[M_IND+i] - current[M_IND+i];
        }
        ...
        for i in 0..HASH_DIGEST_WIDTH{
            result.agg_constraint(
                M_COM_ASSERT + i, 
                mcom_flag.into(), 
                are_equal(current[HASH_IND+i],current[M_IND+i])
            );
        }
    ...
    fn get_assertions(&self) -> Vec<Assertion<Self::BaseField>> {
        ...
        for i in HASH_DIGEST_WIDTH+1..HASH_STATE_WIDTH{
            main_assertions.push(Assertion::single(HASH_IND+i, S_BALL_START, BaseElement::ZERO));
        } 
        ...
        for i in 0..C_SIZE{
            main_assertions.push(Assertion::single(C_IND+i, S_BALL_START + HASH_CYCLE_LEN-1, BaseElement::ZERO));
        }
...
fn get_qr_mask() -> Vec<BaseElement> {
    ...
    for i in S_BALL_START + HASH_CYCLE_LEN-1..(HASH_CYCLE_LEN)*(S_BALL_END){
...
fn get_notqr_mask() -> Vec<BaseElement> {
    ...
    for i in (S_BALL_START + HASH_CYCLE_LEN-1)..(HASH_CYCLE_LEN)*(S_BALL_END)-1{
...
fn get_qrdec_mask() -> Vec<BaseElement> {
    ...
    for i in S_BALL_START + HASH_CYCLE_LEN..(HASH_CYCLE_LEN)*(S_BALL_END)-1 {
...
fn get_qr_base_constants() -> Vec<BaseElement> {
    ...
    for i in S_BALL_START + HASH_CYCLE_LEN-1..(S_BALL_END)*HASH_CYCLE_LEN{
        qr_base[i] = BaseElement::new((N-TAU+i-HASH_CYCLE_LEN - S_BALL_START+1) as u32);
    }
...
fn get_hashmask_constants() -> Vec<BaseElement> {
    ...
    for i in S_BALL_START..(HASH_CYCLE_LEN)*(S_BALL_END){
        hashmask_const[i] = HASH_CYCLE_MASK[i%HASH_CYCLE_LEN];
    }
...
fn get_r_mod() -> Vec<BaseElement> {
    ...
    for i in 0..TAU+1{
        ...
        r_mod[S_BALL_START + HASH_CYCLE_LEN+i] = BaseElement::new(256 - (N-TAU+i) as u32);
    }
...
fn get_q_mod() -> Vec<BaseElement> {
    ...
    for i in 0..TAU+1{
        ...
        q_mod[S_BALL_START + HASH_CYCLE_LEN+i] = BaseElement::new(M/(N-TAU+i) as u32);
    }
